...
function buildProductsGrid($productsList){
    ...
    for ($i = 0; $i < $arrlength; $i++) {
        $qtyId = str_replace(' ', '', $productsList[$i]['productName']) . "-quantity";
        ...
        $productsGrid .= "<label for='". str_replace(' ', '', $productsList[$i]['productName']) ."-quantity'>Quantity</label>";
        $productsGrid .= "<input type='number' class='product-quantity-input' id='". str_replace(' ', '', $productsList[$i]['productName']) ."-quantity' name='". str_replace(' ', '', $productsList[$i]['productName'])."-quantity' min='1' max='10' value='1'>";
...
function buildProductsDetails($productsList, $itemNumber){
    ...
    $productCard .= "<label for='". str_replace(' ', '', $productsList[$i]['productName']) ."-quantity'>Quantity</label>";
    $productCard .= "<input type='number' class='product-quantity-input' id='". str_replace(' ', '', $productsList[$i]['productName']) ."-quantity' name='". str_replace(' ', '', $productsList[$i]['productName']) ."-quantity' min='1' max='10' value='1'>";
    $productCard .= "<button class='add-to-cart-button' onclick=ajaxPost('$i','$qtyId','addToCart')>Add to Cart</button>";
    $productCard .= "<hr>";
...
function buildCartGrid($productList, $cartList){
    ...
    foreach ($cartList as $key => $value){
        ...
        $qtySpanId = str_replace(' ', '', $productList[$key]['productName']) . "-qty-span";
        $subtotalSpanId = str_replace(' ', '', $productList[$key]['productName']) . "-subtotal-span";
        $itemPrice = $productList[$key]['productPrice'];
        $lineId = str_replace(' ', '', $productList[$key]['productName']) . "-item-line";
    ...
    $cartGrid .= "</table>";
    $cartGrid .= "<div id='buttons-line'><a href='index.php' class='add-to-cart-button'>Continue Shopping</a><a href='index.php?action=checkout' class='add-to-cart-button' id='checkout-button'>Check Out</a></div>";
